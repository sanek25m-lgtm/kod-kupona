...
function detectCategory(c) {
  const source = String(c.cat || '').trim().toLowerCase();
  const categories = window.COUPON_ROUTES && window.COUPON_ROUTES.categoryMap || {};
  if (Object.prototype.hasOwnProperty.call(categories, source)) return categories[source].toLowerCase();
  const n = (String(c.name || '') + ' ' + String(c.desc || '')).toLowerCase();
...
async function loadData() {
  ...
  if (urlMerchant) currentMerchant = urlMerchant;
  renderAll();
}
...
function renderMerchants() {
  ...
  let html='';
  merchants.forEach(m=>{const count=data.filter(c=>c.merchant===m).length;html+=`<a class="merchant-item" href="${esc(merchantUrl(m))}">${esc(m)} <span style="color:var(--text-secondary);font-size:.875rem;">(${count})</span></a>`});
  div.innerHTML=html;
}
...
document.addEventListener('click', function(e) {
  ...
  if(merchBtn && merchBtn.tagName === 'A')return;
